...
// Convert a block to state array
void AES::blockToState(const uint8_t* block) {
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < Nb; c++) {
...
void AES::stateToBlock(uint8_t* block) {
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < Nb; c++) {
...
void AES::addRoundKey(int round) {
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < Nb; c++) {
            state[r][c] ^= roundKeys[round * Nb * 4 + c * 4 + r];
        }
    }
}
...
// Apply S-box to a word (used in key expansion)
...
void AES::encryptBlock(const uint8_t* plaintext, uint8_t* ciphertext) {
    blockToState(plaintext);
