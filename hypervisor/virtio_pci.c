...
uint64_t virtio_pci_get_notify_addr(struct virtio_pci_dev *dev,
                                    struct virtq *vq)
{
    uint64_t base = dev->pci_dev.hdr.bars[dev->notify_cap->cap.bar]; 
    uint64_t offset = dev->notify_cap->cap.offset + dev->notify_cap->notify_off_multiplier * vq->info.notify_off;
    return base + offset;
}
...
void virtio_pci_set_pci_hdr(struct virtio_pci_dev *dev,
                            ...
                            uint8_t irq_line)
{
    dev->pci_dev.hdr.device_id = device_id;
    dev->pci_dev.hdr.class_id.class_id = class << 8;
    printf("[class] 0x%x, class before: 0x%x, class after: 0x%x\n", dev->pci_dev.hdr.class_id.class_id, class, class << 8);
    dev->pci_dev.hdr.interrupt_line = irq_line;
}
                     ...
                     struct bus *mmio_bus)
{
    ...
    dev->pci_dev.hdr.vendor_id = VIRTIO_PCI_VENDOR_ID;
    dev->pci_dev.hdr.capabilities_ptr = cap_list;
    dev->pci_dev.hdr.header_type = PCI_HEADER_TYPE_NORMAL;
    dev->pci_dev.hdr.interrupt_pin = 1;
    dev->pci_dev.hdr.status = PCI_STATUS_CAP_LIST | PCI_STATUS_INTERRUPT;
